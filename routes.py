...
def category_books(category_name):
    ...
    for user_data in users_data:
        if user_data["email"] == session.get("mail"):
            favorites = user_data.get("favorites", []) 
            for fav in favorites:
                favs.append(fav["_id"])
    return render_template('all_books.html', category_name=category_name, books=books,type="User",favorites = favs)
...
def get_all_book():
    ...
    for user_data in users_data:
        if user_data["email"] == session.get("mail"):
            ...
            for fav in favorites:
                favs.append(fav["_id"])
    return render_template('all_books.html',books=books,type = type,category_name = None,favorites = favs)
...
def remove_favourite(book_id):
    ...
    # get the user's favourite list
    favorites = user.get("favorites", [])
    # if the book  in the user's favourite list, remove it
    favorites = [fav for fav in favorites if book["_id"] != fav["_id"]]
    user["favorites"] = favorites
    with open("static/users.json", "w") as users_file:
        json.dump(users_data, users_file, indent=4)    
    return jsonify({"success": True, "book_id": book_id})
def add_book():
    ...
    if request.method == "POST":
        ...
        #create book 
        file = request.files["thumbnailUrl"]
    ...
    # get the logged in in users favourites list
        for user_data in users_data:
            if user_data["email"] == session.get("mail"):
                ...
                for fav in favorites:
                    favs.append(fav["_id"])
        return render_template('all_books.html', books=query_books, type="User", category_name=None,favorites=favs)
